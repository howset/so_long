...
/*   Updated: 2024/08/02 11:19:32 by hsetyamu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_pos(t_data *gdata, int horizontal, int length)
{
	if (horizontal == 1)
	{
		if (gdata->map_details.map[gdata->map_details.pl_pos_y]
			[gdata->map_details.pl_pos_x + length] == '1')
			return ;
		update_spr(gdata);
